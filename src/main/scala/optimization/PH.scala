...
class PH(private var gradient: Gradient, private var updater: DistUpdater, private var regularizationOptimizer: RegularizationOptimizationDistributed) extends Distributed(gradient,updater) {
...
object PH extends Logging {
  ...
  def run(
    ...
    def runOneIteration(j:Int,_prevIterRdd:Option[RDD[(Vector,Double,Int)]],stopFlag: Boolean): Unit = {
      ...
      regularizationWeights = regularizationOptimizer.compute(regularizationPenalties.asInstanceOf[DenseVector],weights.asInstanceOf[DenseVector],rho,regParam).asInstanceOf[DenseVector]
    ...
    logInfo("PH finished. Last %d losses %s".format(noLossesToReport,stochasticLossHistory.takeRight(noLossesToReport).mkString(", ")))
